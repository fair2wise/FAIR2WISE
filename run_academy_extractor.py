"""
Launch the TermExtractorAgent on an HPC system via Globus Compute + Academy.

Usage:
    python run_academy_extractor.py --data-dir /path/to/pdfs --output /path/to/terms.json

The agent code runs remotely on the Globus Compute endpoint; this script is
the local client that submits, monitors, and retrieves results. Requires a
running local dashboard (`python user_agent_launcher.py`) which writes
`user_agent_handle.pkl`.
"""
...
# The agent class must be importable on the remote endpoint too. This script
# lives at the repo root, so its directory is the repo root.
...
def _load_user_agent_handle() -> Handle:
    """Load the UserAgent handle written by user_agent_launcher.py."""
    try:
        ...
        raise SystemExit(
            ...
            "`python user_agent_launcher.py` and retry."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run term extraction on HPC via Academy")
    parser.add_argument("--data-dir", required=True, help="Path to PDF directory on the HPC endpoint")
    parser.add_argument("--output", required=True, help="Output JSON path on the HPC endpoint")
    ...
    parser.add_argument("--schema-path", default="storage/schema/matkg_schema.yaml", help="Path to LinkML schema (on the endpoint)")
    parser.add_argument("--max-workers", type=int, default=2)
    parser.add_argument("--log-file", default=None, help="Remote log file path (written on the HPC endpoint)")
