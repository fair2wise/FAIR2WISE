...
class Coordinator:
    ...
    async def run(self, questions: List[str]) -> None:
        ...
        async with await Manager.from_exchange_factory(
            ...
            try:
                for q in questions:
                    await self._answer(q, retrieval, download, extractor)
            finally:
                for h in (retrieval, download, extractor):
                    await manager.shutdown(h, blocking=True)

    async def _answer(self, question: str, retrieval, download, extractor) -> None:
        print(f"\n=== Question: {question} ===")
