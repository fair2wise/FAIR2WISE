"""Launch the Academy user agent and local monitoring dashboard."""
...
try:
    ...
    def init_logging() -> None:
        """Configure standard logging for Academy versions without this helper."""
...
async def launch(port: int, token_db: Path = DEFAULT_TOKEN_DB) -> None:
    ...
    async with await Manager.from_exchange_factory(
        ...
        with open("user_agent_handle.pkl", "wb") as handle:
            pickle.dump(agent_hdl.agent_id, handle)
...
def main() -> None:
    ...
    parser.add_argument(
        "-u", "--user_agent_id", help="User Agent will use this ID if specified"
    )
    parser.add_argument(
        "--token-db",
